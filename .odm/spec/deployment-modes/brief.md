...
This spec defines the 4 deployment modes for Core. Each mode targets a different use case and technical comfort level, from zero-configuration desktop usage (bundled with the Tauri App) to self-hosted infrastructure on a Raspberry Pi home server.
...
- Container deployment — an official Docker image under 50 MB runs Core alongside Pocket ID via docker-compose
...
- As a Docker user, I want a docker-compose file that runs Core and Pocket ID together so that I can deploy with a single command.
...
- The system must provide a standalone binary that runs independently on Linux, macOS, and Windows with YAML/env config.
