//! Legacy authentication module migrated from `apps/core/src/auth/`.
//!
//! Provides the `LegacyAuthProvider` trait for swappable auth backends,
//! the `LocalTokenProvider` implementation, OIDC provider, and WebAuthn.
//! HTTP-specific middleware and routes remain in `apps/core/src/auth/`.
