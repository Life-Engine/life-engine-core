...
These rules exist to protect the interoperability contract between Core, the two SDKs, and every plugin in the ecosystem. Because CDM recommended schemas are the shared data language of Life Engine — defined once in `packages/types/` and consumed by every plugin that reads or writes standard data — a schema change that breaks existing plugins would corrupt data or crash plugin code for users who have not yet updated.
...
All CDM schema changes must conform to these rules before merging. Plugin private collection changes follow a subset of these rules as described in the Plugin Private Collection Versioning section below.

Related specifications: [[cdm-specification]] and [[data]].

## Version Numbering

CDM schema versions are not independent artefacts — they are coupled to the SDK semver version. The version lives in the crate version of `packages/plugin-sdk-rs` and the package version of `packages/plugin-sdk-js`. There is no separate schema version number.
...
- **Adding a new CDM collection** — Plugins that do not declare the new collection in their manifest are unaffected.
...
Within a major SDK version, CDM schemas must only grow — never shrink or change existing definitions. This is the additive-only rule:
...
No other changes to CDM schemas are permitted within a major version. This rule is enforced in CI as described in the CI Enforcement section below.
...
- It identifies all changes and classifies each as breaking or non-breaking using the compatibility classification rules above
...
- If the SDK major version has been incremented, the check confirms the migration format is present (see Migration Format below) and passes
...
- **Release** — The new major version (e.g. `2.0.0`) ships with the breaking changes. The new schema files are published at versioned URIs (see JSON Schema File Convention below). Core begins accepting records in both the old and new format during the overlap period.
...
- **Migration runs** — When Core starts up after an SDK upgrade, it runs migration transforms (see Migration Format below) over all records in the relevant collections. Migration is idempotent, so a restart after a partial migration is safe.
- **Deprecation period** — Plugin authors have 12 months to update their plugins. Deprecation notices are published in the CHANGELOG and propagated through SDK annotations (see Deprecation Notices below).
...
For CDM collections, migration transforms are part of the SDK release, not the plugin. SDK maintainers ship the transform alongside the new major version. For plugin private collections, the plugin author ships the transform in their own manifest.
...
- The plugin must follow the same compatibility classification rules as CDM collections when incrementing its own version. A breaking change to a private collection schema requires a plugin major version bump.
...
- Core runs the plugin's migration transforms on startup after a plugin update, using the same idempotent transform API described in the Migration Format section above.
...
JSON Schema files for CDM collections are published at versioned URIs. The `$id` field in each schema file must include the major SDK version:
...
- and so on for each CDM collection
...
The `$schema` meta-schema declaration in all CDM schema files must use `https://json-schema.org/draft/2020-12/schema` for consistency with the plugin manifest schema.
