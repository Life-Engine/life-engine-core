//! Legacy SQLite implementation of the [`StorageAdapter`] trait.
//!
//! Migrated from `apps/core/src/sqlite_storage.rs` as part of the
//! architecture migration (WP 10.4). Uses `rusqlite` (bundled SQLite)
//! with WAL mode for concurrent readers. A broadcast channel publishes
//! [`ChangeEvent`]s on every mutation.
//!
//! New code should use [`crate::document::SqliteDocumentAdapter`] instead.
...
use life_engine_traits::legacy_storage::{
    ComparisonOp, Pagination, QueryFilters, QueryResult, Record, SortDirection, SortOptions,
    StorageAdapter, StorageError as LegacyStorageError,
};

/// Argon2id settings for passphrase-based key derivation.
///
/// Migrated from `apps/core/src/config.rs` to keep the legacy storage
...
impl SqliteStorage {
    ...
    pub fn open_encrypted(
        ...
        let params = life_engine_crypto::Argon2Params {
            memory_kib: argon2_settings.memory_mb * 1024,
            iterations: argon2_settings.iterations,
            parallelism: argon2_settings.parallelism,
        };
        let passphrase_z = zeroize::Zeroizing::new(passphrase.to_string());
        let key = crate::encryption::derive_db_key_with_params(&passphrase_z, path, &params)
...
impl StorageAdapter for SqliteStorage {
    ...
    async fn update(
        ...
        data: Value,
        version: i64,
    ) -> Result<Record, LegacyStorageError> {
        let now = Utc::now();
        let now_str = now.to_rfc3339();
        let data_str = serde_json::to_string(&data)
            .map_err(|e| LegacyStorageError::Other(e.into()))?;
        ...
        if rows == 0 {
            ...
            let exists: bool = conn
                ...
                .map_err(|e| LegacyStorageError::Other(e.into()))?;
            return if exists {
                Err(LegacyStorageError::VersionMismatch)
            } else {
                Err(LegacyStorageError::NotFound)
            };
        }
        ...
        ).map_err(LegacyStorageError::Other)?;
...
mod tests {
    ...
    async fn encrypted_different_argon2_settings_fails() {
        ...
        let tmp = tempfile::NamedTempFile::new().unwrap();
        let settings_a = Argon2Settings {
            memory_mb: 1,
            iterations: 1,
            parallelism: 1,
        };
        let settings_b = Argon2Settings {
            memory_mb: 2,
            iterations: 1,
            parallelism: 1,
        };
