...
- 1.6. WHEN Core is running in bundled mode THEN the system SHALL include the `plugins/` directory with WASM modules in the App bundle.
...
- 2.1. WHEN Core is started in standalone mode THEN the system SHALL read configuration from `~/.config/life-engine/config.toml` (Linux/macOS) or `%APPDATA%\life-engine\config.toml` (Windows), with environment variables overriding file values.
...
- 2.5. WHEN Core is running in standalone mode THEN the `plugins/` and `workflows/` directories SHALL be configurable via `config.toml`.
...
- 3.2. WHEN `docker-compose up` is run with the provided compose file THEN the system SHALL start Core with correct volume mounts for data, plugins, and workflows.
...
- 3.6. WHEN Core runs inside Docker THEN WASM plugins SHALL be mounted from a volume or baked into the image at `/plugins`.
...
- 4.1. WHEN a release build is produced THEN the system SHALL produce a Core binary compiled for the `aarch64-unknown-linux-gnu` target.
- 4.2. WHEN the ARM64 binary is deployed to a Raspberry Pi 4 THEN Core SHALL start and respond to requests with 128 MB or less of available RAM.
...
- 5.5. WHEN Core is bound to a non-localhost address THEN the system SHALL require authentication on all transport endpoints.
...
- 6.1. WHEN environment variables are set alongside a config file THEN environment variables SHALL take precedence over `config.toml` values.
...
- 6.3. WHEN Core starts THEN the system SHALL log the active deployment mode, bind address, active transports, and whether TLS is enabled.
...
- 7.1. WHEN `config.toml` declares a `[transports.rest]` section THEN Core SHALL start the REST transport on the configured port.
- 7.2. WHEN a transport section is absent from `config.toml` THEN that transport SHALL NOT be started.
- 7.3. WHEN multiple transports are configured THEN Core SHALL start all of them and share the same auth module across all.
